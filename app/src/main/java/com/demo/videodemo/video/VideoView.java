...
import com.demo.videodemo.video.controller.BaseVideoController;
...
import com.demo.videodemo.video.abs.PlayerEventListener;
import com.demo.videodemo.video.config.ProgressManager;
import com.demo.videodemo.video.config.VideoViewConfig;
import com.demo.videodemo.video.config.VideoViewManager;
import com.demo.videodemo.video.factory.AndroidMediaPlayerFactory;
...
public class VideoView extends FrameLayout implements MediaPlayerControl, PlayerEventListener {
    ...
    protected AbsPlayerFactory mPlayerFactory;//工厂类，用于实例化播放核心
    @Nullable
    ...
    //播放器的各种状态
    public static final int STATE_ERROR                 = AbstractPlayer.State.ERROR;
    public static final int STATE_IDLE                  = AbstractPlayer.State.IDLE;
    public static final int STATE_PREPARING             = AbstractPlayer.State.PREPARING;
    public static final int STATE_PREPARED              = AbstractPlayer.State.PREPARED;
    public static final int STATE_PLAYING               = AbstractPlayer.State.PLAYING;
    public static final int STATE_PAUSED                = AbstractPlayer.State.PAUSED;
    public static final int STATE_PLAYBACK_COMPLETED    = AbstractPlayer.State.COMPLETED;
    public static final int STATE_BUFFERING             = AbstractPlayer.State.BUFFERING;
    public static final int STATE_BUFFERED              = AbstractPlayer.State.BUFFERED;
    ...
    protected boolean mAutoRotate; // 自动旋转
    ...
    public VideoView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mPlayerFactory = config.mPlayerFactory == null ? AndroidMediaPlayerFactory.create() : config.mPlayerFactory;
...
//        }
    ...
    protected void startPlay() {
        ...
        VideoViewManager.getInstance().addVideoView(this);
    ...
    protected void saveProgress() {
        Log.d(TAG, "saveProgress: " + mCurrentPosition);
    ...
    protected boolean isInIdleState() {
        return mMediaPlayer == null || mCurrentPlayState == STATE_IDLE;
    }
     ...
     *
     * @deprecated 使用 {@link #setPlayerFactory(AbsPlayerFactory)} ()} 代替
     */
    @Deprecated
    ...
    /**
     * 自定义播放核心，继承{@link AbsPlayerFactory}实现自己的播放核心
     */
    public void setPlayerFactory(AbsPlayerFactory playerFactory) {
    ...
    public void onWindowFocusChanged(boolean hasFocus) {
        ...
        if (isInPlaybackState() && (mAutoRotate || mIsFullScreen)) {
            if (hasFocus) {
                postDelayed(() -> mOrientationEventListener.enable(), 800);
            } else {
                mOrientationEventListener.disable();
            }
        }
